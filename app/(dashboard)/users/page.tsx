...
export default function UserManagement() {
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8"> {/* Added responsive padding */}
      {/* Header */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4"> {/* Adjusted for stacking on small screens */}
        <div>
          <h1 className="text-3xl font-bold text-white text-center md:text-left">User Management</h1> {/* Centered on mobile */}
          <p className="text-gray-400 mt-1 text-center md:text-left">Manage customers, drivers, and vendors</p> {/* Centered on mobile */}
        </div>
        <Button className="bg-green-600 hover:bg-green-700 text-white w-full md:w-auto">Add User</Button> {/* Full width on mobile */}
      </div>
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Select value={roleFilter} onValueChange={setRoleFilter}>
              <SelectTrigger className="w-full md:w-[180px] bg-gray-800 border-gray-700 text-white"> {/* Full width on mobile */}
      ...
      <div className="space-y-4">
        {filteredUsers.map((user) => (
          <Card key={user.id} className="bg-gray-900 border-gray-800 hover:border-green-500 transition-colors">
            <CardContent className="pt-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-4"> {/* Stack on very small screens */}
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 bg-green-600 rounded-full flex items-center justify-center flex-shrink-0"> {/* Added flex-shrink-0 */}
                    <User className="h-6 w-6 text-white" />
                  </div>
                  <div>
                    <h3 className="text-white font-medium text-lg">{user.name}</h3>
                    <div className="flex flex-wrap gap-2 mt-1"> {/* Allow badges to wrap */}
                ...
                <div className="text-left sm:text-right w-full sm:w-auto"> {/* Aligned left on mobile, right on desktop */}
                  <div className="flex items-center space-x-1 mb-1 justify-start sm:justify-end"> {/* Aligned left on mobile, right on desktop */}
              ...
              {/* Action Buttons */}
              <div className="flex flex-wrap gap-2 justify-center sm:justify-start"> {/* Allow buttons to wrap and center on mobile */}
                <Dialog>
                  <DialogTrigger asChild>
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-gray-700 text-gray-400 hover:bg-gray-800 w-full sm:w-auto" // Full width on mobile
                  ...
                  <DialogContent className="bg-gray-900 border-gray-800 text-white max-w-sm sm:max-w-xl md:max-w-3xl"> {/* Responsive width */}
                    <DialogHeader>
                      <DialogTitle>User Profile - {user.name}</DialogTitle>
                    </DialogHeader>
                    {selectedUser && (
                      <Tabs defaultValue="profile" className="w-full">
                        <TabsList className="bg-gray-800 border-gray-700 grid grid-cols-3"> {/* Use grid for tabs on mobile */}
                        ...
                        <TabsContent value="profile" className="space-y-4 pt-4"> {/* Added pt-4 */}
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"> {/* Stack on mobile */}
                            <div className="bg-gray-800 p-4 rounded-lg">
                              ...
                              <div className="space-y-2 text-sm">
                                <div className="flex justify-between flex-wrap"> {/* Allow content to wrap */}
                        ...
                        <TabsContent value="activity" className="space-y-4 pt-4"> {/* Added pt-4 */}
                          <div className="bg-gray-800 p-4 rounded-lg">
                        ...
                        <TabsContent value="documents" className="space-y-4 pt-4"> {/* Added pt-4 */}
                          <div className="bg-gray-800 p-4 rounded-lg">
                            ...
                            {selectedUser.role === "Customer" && (
                              <p className="text-gray-400 text-sm">No specific documents required for customers.</p>
                            )}
                          </div>
                        </TabsContent>
                      </Tabs>
                ...
                {/* Edit button is now hidden from all cards - kept as commented out as per original code */}
                {/*
                <Dialog>
                  <DialogTrigger asChild>
                    <Button
                  ...
                  <DialogContent className="bg-gray-900 border-gray-800 text-white max-w-sm sm:max-w-xl md:max-w-2xl">
                    <DialogHeader>
                ...
                */}
                ...
                {/* Suspend/Unblock button is now hidden from all cards - kept as commented out as per original code */}
                {/*
                <Button
                  size="sm"
                  variant="outline"
      ...
      {/* Summary Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4"> {/* Adjusted for 2 columns on small, 4 on large */}
        <Card className="bg-gray-900 border-gray-800">
          <CardContent className="pt-6">
